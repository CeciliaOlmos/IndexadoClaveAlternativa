      ...
      ******************************************************************
       ...
       77  w-opcion pic 9.
           88 salir value 3.
       77  w-cont pic 99 value zero.
       77  w-i pic 99.
       77  no-encontre pic 9 value 0.
       01  tabla-socios.
           03 largo pic 99.
       ...
       361-INICIALIZO-VECTOR.
           PERFORM VARYING w-i FROM 1 BY 1 UNTIL w-i > largo
               MOVE ZERO TO vec-soc-nro(w-i)
               MOVE " "TO vec-soc-nombre(w-i)
           END-PERFORM.
           MOVE 1 TO largo.
       ...
       370-ELEGIR-CODIGO.
           ...
           ELSE
              PERFORM VARYING w-i from 2 by 1 until w-i > largo
               DISPLAY "SOCIO :"vec-soc-nro(w-i)," NOMB"-
           "RE: ",vec-soc-nombre(w-i)
               END-PERFORM
               PERFORM 380-PROCESO-NOMBRE
           END-IF.

       380-PROCESO-NOMBRE.
           PERFORM 385-PIDO-NRO-NOMBRE.
           PERFORM 390-BUSCO-EN-VECTOR.
           PERFORM 395-VALIDO-CODIGO.

       395-VALIDO-CODIGO.
           PERFORM until no-encontre is equal 1
                DISPLAY "NO ELEGISTE EL CODIGO MOSTRADO!!!!!"
                PERFORM 385-PIDO-NRO-NOMBRE
                PERFORM 390-BUSCO-EN-VECTOR
           END-PERFORM.
           move zero to no-encontre.

       385-PIDO-NRO-NOMBRE.
           DISPLAY "Elija numero de socio para mostrar datos".
           ACCEPT soc-nro.

       390-BUSCO-EN-VECTOR.
           PERFORM VARYING w-i FROM 1 BY 1 UNTIL w-i > largo
               IF soc-nro IS EQUAL vec-soc-nro(w-i)
                   PERFORM 363-BUSCO-SOCIO
                   PERFORM 345-MOSTRAR-RESTO
                   MOVE 1 to no-encontre
               END-IF
           END-PERFORM.
